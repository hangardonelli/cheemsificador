...
function proceso() {
  texElement.src = cheemsTexSrc;
  bocaElement.src = cheemsBocaLSrc;
  bocamaskElement.src = cheemsBocamaskLSrc;
  bocaElement.src = cheemsBocaRSrc;
  bocamaskElement.src = cheemsBocamaskRSrc;

  let minR = Math.round(0);
  let minG = Math.round(133);
  let minB = 77;
  ...
  let min = new cv.Mat(YCrcB.rows, YCrcB.cols, YCrcB.type(), [
    minR,
    minG,
    minB,
    0,
  ]);
  let max = new cv.Mat(YCrcB.rows, img.cols, YCrcB.type(), [
    maxR,
    maxG,
    maxB,
    255,
  ...
  while (i < 500) {
    if (
      mask.ucharAt(1000 - i, ptmedio * mask.channels()) != 0 &&
      upperflag === 1
    ) {
      upperborder = 1000 - i;
      upperflag = 0;
  ...
  let bocaLargo = Math.round((((faceLargo * 4) / 5) * 100) / 100);
  let bocaAlto = Math.round((((faceAlto * 5) / 16) * 100) / 100);
  ...
  let posx = auto_x + parseInt(50);
  let posy = auto_y - parseInt(50);
  ...
  while (i < boca.rows) {
    while (j < boca.cols) {
      ...
      pixel = bocamask_centered.ucharPtr(
        i + posy - centery,
        j + posx - centerx
      );
  ...
  let maskmin = new cv.Mat(
    boca_centered.rows,
    boca_centered.cols,
    boca_centered.type(),
    [0, 32, 32, 0]
  );
  let maskmax = new cv.Mat(
    boca_centered.rows,
    boca_centered.cols,
    boca_centered.type(),
    [255, 255, 255, 255]
  ...
  boca.delete();
  bocamask.delete();
  boca_centered.delete();
  bocamask_centered.delete();
  noboca.delete();
  maskmin.delete();
  maskmax.delete();
  bocamasked.delete();
